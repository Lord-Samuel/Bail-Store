...
import type { BaileysEventEmitter, Chat, ConnectionState, Contact, GroupMetadata, PresenceData, WAMessage, WAMessageCursor, WAMessageKey, GroupParticipant } from 'baileys'
...
import { LabelAssociation, LabelAssociationType, MessageLabelAssociation, ChatLabelAssociation } from 'baileys/lib/Types/LabelAssociation'
...
export default (config: BaileysInMemoryStoreConfig) => {
            ...
            switch (type) {
                ...
                case 'notify':
                    for(const msg of newMessages) {
                        ...
                        const list = assertMessageList(jid)
                        // Fix: Use type assertion for WebMessageInfo
                        list.upsert(msg as any, 'append')
					...
					case 'promote':
						for(const participant of metadata.participants) {
							const isInUpdate = participantObjects.some(p => 
								(p.id as unknown as string) === (participant.id as unknown as string)
							)
							if(isInUpdate) {
				    ...
				    case 'remove':
						metadata.participants = metadata.participants.filter(p => {
							const shouldRemove = participantObjects.some(updateParticipant => 
								(updateParticipant.id as unknown as string) === (p.id as unknown as string)
							)
							return !shouldRemove
						})
					break
    ...
    // Fix: Update the filter function for label associations
    const getMessageLabels = (messageId: string) => {
        const associations = labelAssociations
            .filter((la: LabelAssociation) => {
                // Check if it's a MessageLabelAssociation by looking for messageId property
